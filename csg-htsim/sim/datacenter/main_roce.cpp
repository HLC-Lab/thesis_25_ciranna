...
int main(int argc, char **argv) {
    ...
    uint32_t bool_topology = 0;
    // se la topologia è fat tree allora è 0, altrimenti è 1 nel caso in cui sia dragonfly
    ...
    char* tm_file = NULL;
    char* topo_file = NULL;
    //parameter for Dragonfly+ topology
    uint64_t queue_size_threshold = 0;
    uint32_t s_df = 0;
    uint32_t l_df = 0;
    uint32_t h_df = 0;
    uint32_t p_df = 0;
    int link_speed_set = 0;
    int link_latency_set = 0;
    ...
    while (i<argc) {
        if (!strcmp(argv[i],"-o")) {
            ...
            if (!strcmp(argv[i+1],"Ft")){
                bool_topology = 0;
                cout << "topology type FatTreeTopology"<< endl;
            } else if (!strcmp(argv[i+1],"Df+")){
                bool_topology = 1;
                cout << "topology type DragonFlyPlusTopology" << endl;
            } else {
                cout << "Unknown topology type " << argv[i+1] << endl;
                exit_error(argv[0]);
            }
            i++;
            ...
            //questo per ora non mi interessa
            ...
            if (!strcmp(argv[i+1], "perm")) {
                ...
                FatTreeSwitch::set_strategy(FatTreeSwitch::ECMP); // next to do in dragonfly+
            } else if (!strcmp(argv[i+1], "ecmp_ar")) {
                route_strategy = ECMP_FIB;
                path_entropy_size = 1;
                FatTreeSwitch::set_strategy(FatTreeSwitch::ADAPTIVE_ROUTING); // next to do in dragonfly+
            } else if (!strcmp(argv[i+1], "ecmp_host_ar")) {
                route_strategy = ECMP_FIB;
                FatTreeSwitch::set_strategy(FatTreeSwitch::ECMP_ADAPTIVE); // next to do in dragonfly+
                ...
                route_strategy = ECMP_FIB;
                path_entropy_size = 1;
                FatTreeSwitch::set_strategy(FatTreeSwitch::RR); // next to do in dragonfly+
            } else if (!strcmp(argv[i+1], "fpar")) {
                route_strategy = FPAR;
    ...
    case ECMP_FIB:
        if (bool_topology) {
            fprintf(stderr, "Route Strategy is ECMP, but is not implemented on DragonFly+ Topology\n");
            exit(1);
        }
        break;
    case SCATTER_ECMP:
    ...
    case SINGLE_PATH:
        ...
        if (bool_topology) {
            fprintf(stderr, "Route Strategy is SINGLE_PATH, but is not implemented on DragonFly+ Topology\n");
            exit(1);
        }
        break;
    case FPAR:
        if (!bool_topology) {
            fprintf(stderr, "Route Strategy is FPAR, but is not implemented on Fat Tree Topology\n");
            exit(1);
        }
        break;
    case MINIMAL:
        if (!bool_topology) {
            fprintf(stderr, "Route Strategy is MINIMAL, but is not implemented on Fat Tree Topology\n");
            exit(1);
        }
        break;
    case NOT_SET:
    ...
    if (bool_topology){
        ...
        if (topo_file) {
            ...
            linkspeed_bps link_sp = linkspeed;
            simtime_picosec hop_la = hop_latency;
            if (link_speed_set == 0){
                link_sp = 0;
            }
            if (link_latency_set == 0){
                hop_la = 0;
            }
            top = new DragonflyPlusTopology(k_radix,s_df, l_df, h_df, p_df, no_of_nodes, qt, queuesize, qlf, 
                                                &eventlist, topo_type, queue_size_threshold, no_parallel_link, link_sp, hop_la, switch_latency);
        }
        if (log_switches) {
