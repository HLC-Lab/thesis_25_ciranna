...
void RoceSrc::send_packet() {
    ...
    p = RocePacket::newpkt(_flow, *_route, _highest_sent+1, _mss, false, last_packet,_dstaddr,_srcaddr);
    ...
    new LoggedPacket(parsed_nodename,std::to_string(timeAsUs(eventlist().now())),std::to_string(p->data_packet_size()));
...
void RoceSink::send_ack(simtime_picosec ts) {
    ...
    new LoggedPacket(parsed_nodename,std::to_string(timeAsUs(ts)),std::to_string(ack->data_packet_size()));
...
void RoceSink::send_nack(simtime_picosec ts, RocePacket::seq_t ackno) {
    ...
    new LoggedPacket(parsed_nodename,std::to_string(timeAsUs(ts)),std::to_string(nack->data_packet_size()));
