...
class Packet {
 ...
 public:
    ...
    //questo è strip_payload che c'era prima, ma dava problemi con ndppacket che lo chiamava senza argomenti
    virtual void strip_payload() { assert(!_is_header); _is_header = true;};

    //questo è quello inserito dal ragazzo che ha fatto dragonfly
    ...
    virtual void go_up(){_direction = UP;}
    virtual void go_down(){_direction = DOWN;}
    ...
    virtual uint32_t get_channel() {return _channel;}
 ...
 protected:
    ...
    uint32_t _channel; // used to avoid loop in DragonFly+
